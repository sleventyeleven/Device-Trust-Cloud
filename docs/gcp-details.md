# Google Cloud: Architecture Details, Security, and Troubleshooting

This is the Google Cloud specific companion to the main [Readme.md](../Readme.md).
Everything here is GCP implementation detail, deep troubleshooting, and the
debugging history behind non-obvious decisions in this stack - if you just
want to deploy and enroll a device, the main Readme covers that. Come here
when something breaks, when you want to understand *why* a module is built
the way it is, or before you run `terraform destroy`.
...
The main Readme covers the commands to run. This section covers *why* they
work the way they do.

### Windows: the `-KeyProtection` trade-off, in full
...
this check automatically after installing the certificate, and print
...
### macOS: the keychain cleanup story
...
- **Install scripts are best-effort** - see the main Readme's Certificate
  Enrollment section.
- **macOS keychain installation requires interactive authentication (password
  ...
  prompt(s) when it runs. Installing the root CA, the intermediate CA, and
  the device certificate are three separate keychain-modifying operations,
  and macOS prompts for authentication on each one individually - there's no
  way to bundle them into a single prompt. The script checks first whether
  each CA is already trusted (`security verify-cert`) and skips straight
  past it if so, so a renewal run only re-triggers the one prompt for the
  device certificate itself.
...
### Windows: `-encryption-algo`'s value silently ignored, or a certificate comes back with a garbled subject (e.g. `OU=-encryption-algo`)

The same PowerShell empty-string-argument-dropping quirk described above,
resurfacing for a different pair of flags. If `-Country`/`-Organization`/`-Ou`
are left empty (no default was set for them until this was found and fixed),
PowerShell drops those empty values before they reach `scepclient.exe`, but
the `-country`/`-organization`/`-ou` flag *names* still go through -
shifting every argument after them over by one position each. Depending on
exactly which arguments are empty, this can eat `-encryption-algo`'s own
value entirely, silently falling back to `DES-CBC` with no visible error
(surfacing later as `Unsupported algorithm: 1.3.14.3.2.7` against AWS), or
produce a certificate whose subject contains literal flag names instead of
real values (confirmed live: `CN=<host>,OU=-encryption-algo,O=scep-client,
C=-ORGANIZATION`).

Fixed the same way as the PFX password bug: `install-windows.ps1` now
defaults `$Country`/`$Organization`/`$Ou` to `US`/`DeviceTrust`/`DeviceTrust`
if the corresponding environment variable isn't set, so they're never
actually empty by the time they reach `scepclient`. If you still hit this,
you're on an older copy - re-download it.

**A related trap this can leave behind**: `scepclient` only regenerates
`csr.pem` when there's no existing private key file. If a run fails
*after* generating a key (any failure past that point, not just this one)
and you then retry with different/corrected parameters, `scepclient` will
silently reuse the *stale* cached CSR from the failed attempt - built from
the old parameters - rather than building a new one from your corrected
command line. The retry will report success, but the resulting
certificate's subject still reflects whatever was in that stale CSR. If a
retry succeeds but the certificate's subject doesn't match what you just
passed, delete `client.key`, `csr.pem`, and `self.pem` from the cert
directory first and re-run from a clean state.
...
Use `sudo -E ./install-macos.sh` (see the main Readme's Certificate
Enrollment section). Separately: always invoke it as `./install-macos.sh`
(after `chmod +x`) or `bash install-macos.sh`, matching its `#!/bin/bash`
shebang, not `sh install-macos.sh` - macOS's `sh` is bash running in POSIX
mode, which handles some things (color output included) differently than
the script expects.
...
Limitations above), **CA pools and certificate templates also have their
...
After a `terraform destroy`, a few GCP resources are worth checking by hand
since they either sit outside Terraform state or have their own deletion
quirks:

- **The Terraform state backend bucket** (`google_storage_bucket` referenced
  in the `gcs` backend block, if you're using one) is never touched by
  `destroy` - it holds the state file `destroy` itself needs to run.
- **Orphaned VPC connectors**: if this project or an earlier revision of it
  ever provisioned a `google_vpc_access_connector` that later errored out or
  was removed from the Terraform config without a clean destroy, it can be
  left behind untracked, and its auto-created firewall rule can block
  `google_compute_network` deletion on a future destroy with an error that
  doesn't mention the connector by name. Check with `gcloud compute networks
  vpc-access connectors list --region=<region>` and delete any leftover
  connector directly if `destroy` fails on the network with a firewall-rule
  ownership error.
- A quick sweep worth running after any destroy you expect to be final:
  `gcloud compute instances list`, `gcloud privateca pools list --location=<region>`,
  and `gcloud secrets list` should all come back empty for this project.
